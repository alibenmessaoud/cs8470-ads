...
is used to describe the situation that occurs when multiple data sources
with incompatible descriptions (schemas, ontologies etc.) must be integrated
for use by a single application. This problem is often encountered in 
the semantic web. A fundamental step in solving the heterogeneous data problem
is the production of an alignment, that can tell the client how to equate
...
\begin{figure*}
%\centering
%\resizebox{!}{0.35\textwidth}{\small{\input{fmeasure.tex}}}
\input{allcompare_5000.tex}

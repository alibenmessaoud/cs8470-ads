...
//      dump(pgnodes.values());
    ...
    if (TEST)
      System.err.print ("Iterating over (" +
			m1.size () + " arcs, " +
			RDFUtil.getNodes (m1).size () + " nodes) x (" +
			m2.size () + " arcs, " +
			RDFUtil.getNodes (m2).size () + " nodes): ");
...
//     }
  ...
  public double distanceF (PGNode[]nodes, double maxN, double maxN1)
  {
    ...
    if (DEBUG)
      System.err.println ("Calc distance with maxN=" + maxN + ", maxN1=" +
			  maxN1);
...
//      }
		...
		StmtPair p = new StmtPair (st1, st2, ps,
					   getCard (cardMapSPLeft,
						    ...
						    st1.predicate ()),
					   getCard (cardMapOPLeft,
						    st1.object (),
						    ignorePredicates ? null :
						    st1.predicate ()),
					   getCard (cardMapPLeft, null,
						    ignorePredicates ? null :
						    st1.predicate ()),
					   getCard (cardMapSPRight,
						    st2.subject (),
						    ignorePredicates ? null :
						    st2.predicate ()),
					   getCard (cardMapOPRight,
						    st2.object (),
						    ignorePredicates ? null :
						    st2.predicate ()),
					   getCard (cardMapPRight, null,
						    ignorePredicates ? null :
						    st2.predicate ()));
...
//        MapPair p = new MapPair(st1, st2, ps);
		...
		if (FLOW_GRAPH_TYPE == FG_STOCHASTIC
		    || FLOW_GRAPH_TYPE == FG_INCOMING)
		  {

		    // collect the numbers

		    MapPair sourcePair =
		      get (outgoing, st1.subject (), st2.subject ());
		      sourcePair.sim += 1.0;

		      sourcePair =
		      get (outgoing, st1.object (), st2.object ());
		      sourcePair.sim += 1.0;

		    if (TRY_ALL_ARCS)
		      {
			sourcePair =
			  get (outgoing, st1.subject (), st2.object ());
			sourcePair.sim += 1.0;
...
//          targetPair.sim += 1.0;
	...
	while (it.hasNext ())
	  {
	    ...
	    p.soso =
	      1.0 / get (outgoing, p.stLeft.subject (),
			 p.stRight.subject ()).sim;
	    p.osos =
	      1.0 / get (outgoing, p.stLeft.object (),
			 p.stRight.object ()).sim;
	    if (TRY_ALL_ARCS)
	      {
		p.soos =
		  1.0 / get (outgoing, p.stLeft.subject (),
    ...
    cardMapSPLeft = cardMapOPLeft = cardMapSPRight = cardMapOPRight =
      cardMapPLeft = cardMapPRight = null;
...
//     pgarcs = new ArrayList();
  ...
  void computeCardMaps (Model m, Map cardMapSP, Map cardMapOP, Map cardMapP,
  ...
  class StmtPair
  {
    ...
    public StmtPair (Statement stLeft, Statement stRight, double predSim,
		     int spLeft, int opLeft, int pLeft, int spRight,
		     int opRight, int pRight)
    {
	...
	case Match.FG_STOCHASTIC:
	  {			// for constant weight, weight computed here does not matter...
	...
	return "StmtPair[(" + stLeft.subject () + "," + stRight.subject () +
	  ") -> (" + stLeft.object () + "," + stRight.object () + "), " +
	  soso + "->, <-" + osos + (TRY_ALL_ARCS ? " *** (" +
				    stLeft.subject () + "," +
				    stRight.object () + ") -> (" +
				    stLeft.object () + "," +
				    stRight.subject () + "), " + soos +
				    "->, <-" + osso + ")" : "");
      }
  ...
  static void OAEIStandardRun (String ontAfile,
			       String ontBfile) throws Exception
  {
    System.err.
      println
    ...
    //GET DOMAINS AND RANGES
      Set < OWLObjectProperty > objPropSig =
      basic_onto.getObjectPropertiesInSignature (false);
    for (OWLObjectProperty objProp:objPropSig)
      {
	Set < OWLClassExpression > objPropDomains =
	  objProp.getDomains (basic_onto);
	Set < OWLClassExpression > objPropRanges =
	  objProp.getRanges (basic_onto);

	System.out.printf ("%s: domains = %d; ranges = %d\n",
			   prettyID (objProp.toStringID ()),
			   objPropDomains.size (), objPropRanges.size ());
	...
	for (OWLClassExpression domain:objPropDomains)
	  {
		...
		if (domainName != null
		    && !resources.keySet ().contains (domainName))
		  {
		    resources.put (domainName,
				   nf.createResource (domainName));
		  }
		for (OWLClassExpression range:objPropRanges)
		  {
		    String rangeName =
		      prettyID (range.asOWLClass ().getIRI ().toString ());
		    if (rangeName != null
			&& !resources.keySet ().contains (rangeName))
		      {
			resources.put (rangeName,
				       nf.createResource (rangeName));
		      }
		    if (domainName != null && rangeName != null)
		      {
			ontology_A.add (nf.
					createStatement (resources.
							 get (domainName), p,
							 resources.
							 get (rangeName)));
		      }
			...
			System.out.printf ("%s | %s | %s\n",
					   prettyID (domain.asOWLClass ().
						     getIRI ().toString ()),
					   prettyID (objProp.toStringID ()),
					   range.asOWLClass ().getIRI ().
					   toString ());
		      }
		  }
	      }
		...
		if (DEBUG == true)
		  {
		    System.out.printf ("%s isA %s\n",
				       prettyID (owlSubclass.asOWLClass ().
						 getIRI ().toString ()),
				       prettyID (owlClass.getIRI ().
						 toString ()));
		  }
	      }
  ...
  static void orderedNodesExample (int num) throws Exception
  {
    ...
    MapPair.printMap (new FilterBest ().
		      getFilterBest (Arrays.asList (result), true),
		      System.out);
  }

  static void addSequence (Model m, NodeFactory f, String str,
			   String prefix) throws ModelException
  {
  ...
  static void sequenceExample (String seq1, String seq2) throws Exception
  {
    ...
    List pruned =
      new FilterBest ().getFilterBest (Arrays.asList (result), true);
